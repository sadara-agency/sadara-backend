...
router.get('/', validate(contractQuerySchema, 'query'), asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const countResult = await query(`SELECT COUNT(*) FROM contracts c JOIN players p ON c.player_id = p.id ${where}`, values);
...
router.get('/:id', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (result.rows.length === 0) throw new AppError('Contract not found', 404);

  const milestones = await query(
    `SELECT ms.* FROM milestones ms JOIN commission_schedules cs ON ms.commission_schedule_id = cs.id
     WHERE cs.contract_id = $1 ORDER BY ms.due_date`, [req.params.id]
  );

  sendSuccess(res, { ...result.rows[0], milestones: milestones.rows });
}));
...
router.post('/', authorize('Admin', 'Manager'), validate(createContractSchema), asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const result = await query(
    ...
    [d.playerId, d.clubId, d.category, d.title, d.startDate, d.endDate, d.baseSalary, d.salaryCurrency,
     d.signingBonus, d.releaseClause, d.performanceBonus, d.commissionPct, totalCommission, req.user!.id]
  );

  await logAudit('CREATE', 'contracts', result.rows[0].id, buildAuditContext(req.user!, req.ip), `Created contract: ${d.title}`);
