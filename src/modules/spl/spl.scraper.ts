...
import { ScrapedPlayerBio, ScrapedSeasonStats, ScrapedCareerEntry, ScrapedPlayerFull } from './spl.types';
...
function parseHeight(raw: string | null): number | null {
  ...
  const n = raw.match(/([\d.]+)/);
  if (n) { const v = parseFloat(n[1]); return v < 3 ? Math.round(v * 100) : Math.round(v); }
  return null;
}
...
export async function scrapePlayerProfile(splPlayerId: string, slug = 'player'): Promise<ScrapedPlayerFull | null> {
  const url = `${SPL_BASE}/en/players/${splPlayerId}/${slug}`;
  try {
    const { data: html } = await axios.get(url, {
      timeout: REQUEST_TIMEOUT,
      headers: { 'User-Agent': 'Sadara-Sports-Platform/1.0 (data-sync)', Accept: 'text/html', 'Accept-Language': 'en' },
    });
    const $ = cheerio.load(html);
    const bio = extractBio($, splPlayerId, slug);
    if (!bio.fullName) { console.warn(`[SPL] No name for ${splPlayerId}`); return null; }
    return { bio, currentSeasonStats: extractStats($), careerHistory: extractCareer($), scrapedAt: new Date() };
  } catch (err: any) {
    if (err.response?.status === 404) { console.warn(`[SPL] 404 for ${splPlayerId}`); return null; }
    throw err;
  }
}
...
function extractBio($: cheerio.CheerioAPI, splPlayerId: string, slug: string): ScrapedPlayerBio {
  ...
  const jerseyText = $('[class*="jersey"], [class*="number"], [class*="shirt"]').first().text().trim();
  const jerseyNumber = jerseyText ? parseInt(jerseyText.replace(/\D/g, ''), 10) || null : null;

  const info = new Map<string, string>();
  $('[class*="info"] li, [class*="detail"] li, [class*="meta"] li, dl dt, table.info tr').each((_, el) => {
    const label = sanitize($(el).find('dt, th, label, [class*="label"], span:first-child').text()).toLowerCase();
    const value = sanitize($(el).find('dd, td, [class*="value"], span:last-child').text());
    if (label && value && label !== value) info.set(label, value);
  });

  let photoUrl: string | null = null;
  $('img[src*="pulselive"], img[src*="headshot"], img[class*="player-image"]').each((_, img) => {
    const src = $(img).attr('src') || $(img).attr('data-src');
    if (src && (src.includes('headshot') || src.includes('player')))
      photoUrl = src.startsWith('http') ? src : `https://static-files.saudi-pro-league.pulselive.com${src}`;
  });
  ...
  return {
    splPlayerId, slug, fullName, jerseyNumber,
    nationality: info.get('nationality') || null,
    position: info.get('position') || null,
    dateOfBirth: parseDateOfBirth(info.get('date of birth') || info.get('born') || null),
    heightCm: parseHeight(info.get('height') || null),
    photoUrl, pulseLiveId: extractPulseLiveId(photoUrl),
...
function extractStats($: cheerio.CheerioAPI): ScrapedSeasonStats | null {
  ...
  const get = (...keys: string[]) => { for (const k of keys) if (map.has(k)) return map.get(k)!; return 0; };
  return {
    season: '2025/26',
...
function extractCareer($: cheerio.CheerioAPI): ScrapedCareerEntry[] {
  ...
  $('table').each((_, table) => {
    const hdrs = $(table).find('th').map((_, th) => sanitize($(th).text()).toLowerCase()).get();
    const si = hdrs.findIndex(h => h.includes('season') || h.includes('year'));
...
export async function scrapeTeamRoster(splTeamId: string): Promise<Array<{ splPlayerId: string; slug: string; name: string }>> {
  const players: Array<{ splPlayerId: string; slug: string; name: string }> = [];
  try {
    const { data: html } = await axios.get(`${SPL_BASE}/en/teams/${splTeamId}`, {
      timeout: REQUEST_TIMEOUT,
      headers: { 'User-Agent': 'Sadara-Sports-Platform/1.0 (data-sync)', Accept: 'text/html' },
    });
    const $ = cheerio.load(html);
    $('a[href*="/en/players/"]').each((_, el) => {
      const parsed = extractSplId($(el).attr('href') || '');
      if (parsed && !players.find(p => p.splPlayerId === parsed.id))
        players.push({ splPlayerId: parsed.id, slug: parsed.slug, name: sanitize($(el).text()) || parsed.slug.replace(/-/g, ' ') });
    });
...
export async function scrapePlayers(
  ...
  for (let i = 0; i < list.length; i++) {
    try {
      ...
      const r = await scrapePlayerProfile(list[i].splPlayerId, list[i].slug || 'player');
      results.push(r);
