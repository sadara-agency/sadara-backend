...
export const syncAllSchema = z.object({
  confirm: z.literal(true, { errorMap: () => ({ message: 'Set confirm: true to sync all 18 teams' }) }),
});

export const seedClubIdsSchema = z.object({
  confirm: z.literal(true, { errorMap: () => ({ message: 'Set confirm: true to seed club IDs' }) }),
});
