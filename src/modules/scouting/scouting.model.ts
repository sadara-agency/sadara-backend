...
Watchlist.init(
  {
    ...
    status: {
      type: DataTypes.ENUM("Active", "Shortlisted", "Archived", "Rejected"),
      defaultValue: "Active",
    },
...
export class ScreeningCase
  extends Model<ScreeningCaseAttributes, ScreeningCreationAttributes>
  implements ScreeningCaseAttributes
{
  declare id: string;
...
ScreeningCase.init(
  {
    ...
    status: {
      type: DataTypes.ENUM("InProgress", "PackReady", "Closed"),
...
export class SelectionDecision
  extends Model<SelectionDecisionAttributes, SelectionCreationAttributes>
  implements SelectionDecisionAttributes
{
  declare id: string;
...
SelectionDecision.init(
  {
    ...
    decision: {
      type: DataTypes.ENUM("Approved", "Rejected", "Deferred"),
      allowNull: false,
    },
    decisionScope: {
      type: DataTypes.ENUM("Full", "Transfer-Only"),
      defaultValue: "Full",
...
Watchlist.hasMany(ScreeningCase, {
  foreignKey: "watchlistId",
  as: "screeningCases",
});
ScreeningCase.belongsTo(Watchlist, {
  foreignKey: "watchlistId",
  as: "watchlist",
});
ScreeningCase.hasMany(SelectionDecision, {
  foreignKey: "screeningCaseId",
  as: "decisions",
});
SelectionDecision.belongsTo(ScreeningCase, {
  foreignKey: "screeningCaseId",
  as: "screeningCase",
});
